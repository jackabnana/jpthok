...
$user = new Admin();
$currentpage = 'store_detail.php';
$currentpagetitle = 'Store detail';
$getdata = $get->get_active_data($_REQUEST['id'],'store_details');
...
if(isset($_POST['edit']))
{
	  $edit = $set->update_store_detail($_REQUEST['id']);
}

if(isset($_POST['doaction'])){
	extract($_POST);
$add = $set->do_action($action,$ids,'store_details');
}
...
<div class="dashbox-main-div">
	<?php if($msg != "") { echo '<p class="success">'.$msg.'</p>'; } ?>
		...
		<tr>
		<td>Display name</td><td><input type="text" name="dis_name" value="<?=$fetchrow->dis_name?>" /></td>
		</tr>
		<tr>
		<td>Description</td><td><textarea name="description"><?=$fetchrow->description?></textarea></td>
		</tr>
		<tr>
...
<?
$selectcat = mysql_query("SELECT * FROM  store_details ORDER BY id DESC LIMIT $start, $limit");
?>
