...
//echo sizeof($_FILES['prodcut_img']['name']);die;
for($a=0;$a<sizeof($_FILES['prodcut_img']['name']);$a++)
{
$filename = $_FILES['prodcut_img']['tmp_name'][$a];	
$array_message[] = $get->validate_image($filename,640,480,1024);
$set_array[] =key($array_message[$a]); 

if(key($array_message[$a])=='error'){
$pos_val[] = $a;	
$imp = implode(",",$pos_val);	
}
...
//print_r($pos_val);
//echo $dataval =  key($array_message);
//echo $array_message[$dataval];
//print_r($array_message); echo "<br><br>hhhh<br>";print_r($set_array);

if (!in_array("error", $set_array))
{
  
	$edit = $set->update_product($_REQUEST['id'],$_REQUEST['request_prod_id'],$_REQUEST['page']);
    
}else{
$error_message = "Image dimensions are too small. Upload Image Minimum width 640px And Minimum height is 480px And Size is Less than 2Mb"."Error Image Position is=> ".$imp;	
...
echo '<p class="error">'.$error_message.'</p>'; } else{ echo '<p class="error">Upload Image Minimum width 640px And Minimum height is 480px And Size is Less than 2Mb </p>'; } ?>
