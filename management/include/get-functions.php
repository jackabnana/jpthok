...
//Fetch Whatsup No
	public function get_whatsup_no(){
		$sql = "SELECT * FROM global_setting WHERE id=1";
		$query = mysql_query($sql);
		$row = mysql_fetch_array($query);
		return $row['whatsup_no'];
	}
...
/******************Get stock attribute*********************/			
	...
	public	function get_stock_attribute_user($pid)
	{			
	
		$select_product = mysql_query("SELECT * FROM  product_attribute WHERE prod_id='$pid' and attri_group_id!=0 LIMIT 0,2");
		$count = mysql_num_rows($select_product);
		if($count > 0){ return $select_product; } else { return 'no'; }
    }
...
/* VENDOR CODE START */
	...
	public function get_commission_according_category($pid)
	{
		
		$sql = "SELECT cat_id, subcat_id ,sub_subcat_id FROM product WHERE prod_id = '$pid' ";
		$query = mysql_query($sql);
		$row = mysql_fetch_array($query);
		...
		if($row['sub_subcat_id']!='')
		{
			...
			$cat = mysql_query("SELECT commission FROM category WHERE category_id = '".$row['sub_subcat_id']."' ");
			$row_cat = mysql_fetch_array($cat);
			return $row_cat['commission'];
		}
		elseif($row['subcat_id']!='')
		{
			
			$cat = mysql_query("SELECT commission FROM category WHERE category_id = '".$row['subcat_id']."' ");
			$row_cat = mysql_fetch_array($cat);
			return $row_cat['commission'];
		...
		elseif($row['cat_id']!='')
		{
			...
			$cat = mysql_query("SELECT commission FROM category WHERE category_id = '".$row['cat_id']."' ");
			$row_cat = mysql_fetch_array($cat);
			return $row_cat['commission'];
	...
	//Fetch Category By Id
	public function get_active_data_vendor($id,$tbname,$vendor)
	{
	$sql = "SELECT * FROM $tbname WHERE id='$id' AND vendor_id = '".$vendor."' ";
	$query = mysql_query($sql);
	return $query;
	}

	function get_total_product_vendor($vendor)
	{
		$result=mysql_query("select * from product WHERE vendor_id = '".$vendor."' ");
		$count=mysql_num_rows($result);
		return round($count);
	}
	
	function get_total_order_vendor($vendor)
	{
		$result = mysql_query("select * from order_prodcut_details WHERE vendor_id = '".$vendor."' GROUP BY orderid ");
		$count=mysql_num_rows($result);
		return round($count);
	}
	
	//Get Total Complete Order
	function get_total_panding_order_vendor($vendor){
		$result=mysql_query("select * from order_details WHERE vendor_id = '".$vendor."' AND (status='pending' OR status = 'payment_pending')");
		$count=mysql_num_rows($result);
		return round($count);
	}
	
	//Get Total Complete Order
	function get_total_complete_order_vendor($vendor){
		$result=mysql_query("select * from order_details WHERE vendor_id = '".$vendor."' AND  status='complete'");
		$count=mysql_num_rows($result);
		return round($count);
	}

	public function get_pagination_vendor($tbname,$currentpage,$page,$start,$limit,$vendor)
	{
	$rows=mysql_num_rows(mysql_query("select * from $tbname WHERE vendor_id = '".$vendor."' "));
	$total=ceil($rows/$limit);
	$pagination .= '<div class="bodyfooter fright"><br /><div class="fleft">';
	$url = ADMIN_PATH."/".$currentpage."?role=view";
	if($page>1)
...
public function get_all_pagination_vendor($tbname,$currentpage,$page,$start,$limit)
{
	$rows=mysql_num_rows(mysql_query("select * from $tbname WHERE vendor_id > 0 "));
	$total=ceil($rows/$limit);
	$pagination .= '<div class="bodyfooter fright"><br /><div class="fleft">';
	$url = ADMIN_PATH."/".$currentpage."?role=view";
	if($page>1)
...
public function get_vendor_name($id)
	{
		$sql = mysql_query("SELECT * FROM admin_login WHERE vendor_id = '".$id."' ");
		$result = mysql_fetch_array($sql);
	...
	{
		$sql = mysql_query("SELECT * FROM store_details WHERE vendor_id = '".$id."' ");
		$result = mysql_fetch_array($sql);
		$address = $result['dis_name'].':'.$result['description'];	
		return $address;
	}
...
function get_order_date($oid)
{
	$sql = mysql_query("SELECT orderdate FROM order_details WHERE orderid = '$oid' ");
	$result = mysql_fetch_array($sql);
	return $result['orderdate'];
	
}

function get_order_total_vendor($vId,$oId)
{
	$fetch = mysql_query("SELECT * FROM order_prodcut_details WHERE vendor_id= '".$vId."' AND orderid = '".$oId."' ");
	while($row_fetch = mysql_fetch_array($fetch))
	{
		$price = ($this->get_product_price($row_fetch['prod_id']))*($row_fetch['qty']);
		$Tprice += $price;
	}
	return $Tprice;
}

function get_order_product_price_vendor($vId,$oId,$pid)
{
	$fetch = mysql_query("SELECT * FROM order_prodcut_details WHERE vendor_id= '".$vId."' AND orderid = '".$oId."' ");
	$row_fetch = mysql_fetch_array($fetch);
	return $price = $this->get_product_price($pid)*$row_fetch['qty'];
}




function get_product_id_vendor($vId,$oId)
{
	
	$vend = mysql_query("SELECT * FROM order_prodcut_details WHERE orderid = '$oId' AND vendor_id = '$vId' ");
	while($res = mysql_fetch_array($vend))
	{
	    $Tprice = $this->get_order_product_price_vendor($vId,$oId,$res['prod_id']);
		$comm = $this->get_commission_according_category($res['prod_id']);
	    $per_comm = ($comm*$Tprice)/100;
		$cat_comm += $per_comm;
	}
	return $cat_comm;
}
...
function get_order_total_tax($vId,$oId)
{
	
    $cat_comm = $this->get_product_id_vendor($vId,$oId);
	
	$Tprice = $this->get_order_total_vendor($vId,$oId);
	$Collection_fee = $this->get_collection_fee();
	$fixed_fee = $this->get_fixed_fee();
	$Service = $this->get_service_tax();
	$rupee_Service = ($Tprice*$Service)/100;

	$SwachhBharat = $this->get_swachh_bharat();
	$rupee_SwachhBharat = ($Tprice*$SwachhBharat)/100;
	return $total_tax = $cat_comm + $Collection_fee + $fixed_fee + $rupee_Service + $rupee_SwachhBharat;
	//$commission = $this->get_commission_according_category($pid)
}
function get_payment_method($oId)
{
	$sql = mysql_query("SELECT payment_method FROM order_details WHERE orderid = '$oId' ");
	$res = mysql_fetch_array($sql);
	return $res['payment_method'];
	
}
function get_payment_status($oId)
{
	$sql = mysql_query("SELECT payment_status FROM order_details WHERE orderid = '$oId' ");
	$res = mysql_fetch_array($sql);
	return $res['payment_status'];
	
}

function get_order_tracking_status($oId)
{
	
	$sql = mysql_query("SELECT status FROM order_tracking WHERE orderid = '$oId' ");
	$res = mysql_fetch_array($sql);
	return ucfirst($res['status']);
	
}
function get_order_attribute_id($oid,$vid,$pid,$z)
{
	
	$selectorderattribute = "SELECT * FROM order_prodcut_attribute WHERE orderid='$oid' AND attribute_id > 0 AND prod_id ='$pid' ";
	$queryc = mysql_query($selectorderattribute);
	$res = mysql_fetch_array($queryc);
...
function get_order_attribute_option_id($oid,$vid,$pid,$z)
{
	
	$selectorderattribute = "SELECT * FROM order_prodcut_attribute WHERE orderid='$oid' AND attribute_id > 0 AND prod_id ='$pid' LIMIT $z,1 ";
	$queryc = mysql_query($selectorderattribute);
	$res = mysql_fetch_array($queryc);
	$row_c = mysql_num_rows($queryc);
	if($row_c > 0)
	{
		...
		return $res['attribute_option_id'];	
	}
	
}

function get_number_of_month_acc_order($vid,$month)
{
	
	$order_new = mysql_query("SELECT * FROM order_prodcut_details WHERE vendor_id='".$vid."' AND DATE_FORMAT(orderdate, '%M') ='".$month."' GROUP BY orderid ");
	return $row_c = mysql_num_rows($order_new);
	
}
//Fetch Delivery Charges
public function get_individual_delivery_charges()
{
	$sql = "SELECT * FROM global_setting WHERE id=1";
	$query = mysql_query($sql);
	$row = mysql_fetch_array($query);
...
public function get_total_delivery_charges($oid)
{
	$query = mysql_query("SELECT * FROM order_prodcut_details WHERE orderid = '$oid' ");
	$total = '';
	while($row = mysql_fetch_array($query))
	{
		
		$total += $row['delivery_charges'];
	}
	return $total;
}

public function get_store_name($vid)
{
	$query = mysql_query("SELECT dis_name FROM store_details WHERE vendor_id = '$vid' ");
	$row = mysql_fetch_array($query);
	return $row['dis_name'];
}
public function get_store_address($vid)
{
	$query = mysql_query("SELECT description FROM store_details WHERE vendor_id = '$vid' ");
	$row = mysql_fetch_array($query);
	return $row['description'];
}


public function get_vendor_total($vid)
{
	//$query = mysql_query("SELECT sum(subtotal) AS total FROM order_prodcut_details WHERE vendor_id = '$vid' ");
	//$row = mysql_fetch_array($query);
	//return $row['total'];
	
	
	$vendor = "SELECT  * FROM order_prodcut_details WHERE vendor_id = '".$vid."' ";
...
public function get_vendor_tax($vid)
{
	$query = mysql_query("SELECT sum(subtotal) AS total FROM order_prodcut_details WHERE vendor_id = '$vid' ");
	$row = mysql_fetch_array($query);
	return $row['total'];
