...
<h2><span>Manage <?=$currentpagetitle?></span>
	<p> 
	<a href="<?=ADMIN_PATH?>/orders.php?role=view&action=track" class="red_button fright">Back</a>
	</p>
</h2>
<div class="cboth"></div>
</div>
  ...
  <table class="product">
	...
	<td><li class="list">
		...
		<div class="total-round">
			 <span class="round  <?if($status == 'approval'){ echo 'checked current-status';}elseif($status == 'processing' || $status == 'dispatch'|| $status == 'complete'){ echo 'checked'; } else{ echo 'active'; }?>">
				<?
				$sql_track = mysql_query("SELECT * FROM order_tracking WHERE orderid = '".$_GET['id']."' and status = 'approval' ORDER BY id DESC");
	...
	<td><li class="list">
		...
		<div class="total-round">
			...
			<?
			$sql_track = mysql_query("SELECT * FROM order_tracking WHERE orderid = '".$_GET['id']."' and status = 'processing' ORDER BY id DESC");
			$order_track  = mysql_fetch_array($sql_track);
			$count = mysql_num_rows($sql_track);
			if($count >0){
			?>
	...
	<div class="line-connect"></div>
		<div class="total-round">
			...
			<?
			$sql_track = mysql_query("SELECT * FROM order_tracking WHERE orderid = '".$_GET['id']."' and status = 'dispatch' ORDER BY id DESC");
			$order_track  = mysql_fetch_array($sql_track);
			$count = mysql_num_rows($sql_track);
			if($count >0){
			?>
	...
	<td><li class="list">
		...
		<div class="line-connect"></div>
			...
			<?
			$sql_track = mysql_query("SELECT * FROM order_tracking WHERE orderid = '".$_GET['id']."' and status = 'complete' ORDER BY id DESC");
			$order_track  = mysql_fetch_array($sql_track);
			$count = mysql_num_rows($sql_track);
			if($count >0){
			?>
	...
	<tr>
	  ...
	  <div style="float:left;width:48.5%;max-height:191px;overflow-y:auto;overflow-x:hidden;">
