...
$user = new Admin();
$currentpage = 'vendor_product.php';
$currentpagetitle = 'Vendor product';
...
<div class="dashbox-main-div">
	...
	?>
		...
		<?
		$z=1;
		$productatt = mysql_query("SELECT *, PDG.id AS pid,DPA.id AS PID FROM  detail_product_attribute AS DPA JOIN product_detail_group AS PDG ON DPA.product_attri_group_id = PDG.id WHERE prod_id='{$_REQUEST['request_prod_id']}'"); ?>
		<table style="width:100%; margin:0;">
		<?
		while($proatt=mysql_fetch_array($productatt))
		{
			  ...
			  <tr id="table<?=$z?>">
				  ...
				  {
				   ?>
				   <option <?if($getrelated['id']==$pid){ ?> selected <? }?> value="<?=$getrelated['id']?>"><?=$getrelated['attri_group_name']?></option><?
				   }
				   ?>
				   </select>
				  </td>
...
$sql = " SELECT P.id as pid,P.status as p_status,P.*,C.* From product as P, 
	category as C  WHERE P.vendor_id > 0  ";
...
<tr>
	...
	<a href="<?=ADMIN_PATH?>/product.php?role=edit&page=<?=$page?>&id=<?=$pid?>&request_prod_id=<?=$prod_id?>" class="edit">
	<img src="images/edit.png" width="35" /> 
	</a>
	</td>
	<td width="50" align="center"><?=$get->get_vendor_name($vendor_id)?></td>
...
<? endif; ?>
	...
	<?=$get->get_all_pagination_vendor('product',$currentpage,$page,$start,$limit)?>
<div  class="cboth"></div>
</form>
	</div>
