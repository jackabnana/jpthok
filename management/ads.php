...
if(isset($_POST['edit'])){
$edit = $set->update_ads($_REQUEST['id']);
}
...
<div class="dashbox-main-div">
	...
	<?php if($msg != "") { echo '<p class="success">'.$msg.'</p>'; } ?>
		...
		<input type="submit" name="<?=$getrole?>" class="green_button fright" value="<?=$getrole?> ads" />
			...
			<tr>
			<td>Image</td>
			<td><img width="70" src="<?=$site_url?>/upload/ads/<?=$fetchrow->ad_img?>"></td>
			</tr>
			<? endif; ?>
			<tr>
			<td>Ads Url </td><td><input type="text" name="ads_url" value="<?=$fetchrow->ad_url?>" /></td>
			</tr>
			<tr>
			<td>Ads Code </td><td><textarea name="ad_code"><?=$fetchrow->ad_code?></textarea></td>
			</tr>
			<tr>
			...
			<option value="s-first" <? if($fetchrow->ad_postion == 's-first') { echo "selected";}?>>Slider First</option>
			<option value="s-second" <? if($fetchrow->ad_postion == 's-second') { echo "selected";}?>>Slider Second</option>
			<option value="s-third" <? if($fetchrow->ad_postion == 's-third') { echo "selected";}?>>Slider Third</option>
			<option value="f-first" <? if($fetchrow->ad_postion == 'f-first') { echo "selected";}?>>FLY WITH THE WORLD! First</option>
			<option value="f-second" <? if($fetchrow->ad_postion == 'f-second') { echo "selected";}?>>FLY WITH THE WORLD! Second</option>
			<option value="f-third" <? if($fetchrow->ad_postion == 'f-third') { echo "selected";}?>>FLY WITH THE WORLD! Third</option>
			<option value="f-forth" <? if($fetchrow->ad_postion == 'f-forth') { echo "selected";}?>>FLY WITH THE WORLD! Forth</option>
			<option value="f-fifth" <? if($fetchrow->ad_postion == 'f-fifth') { echo "selected";}?>>FLY WITH THE WORLD! Fifth</option>
			<option value="right" <? if($fetchrow->ad_postion == 'right') { echo "selected";}?>>Right</option>
			<option value="left" <? if($fetchrow->ad_postion == 'left') { echo "selected";}?>>Left</option>
			<option value="top" <? if($fetchrow->ad_postion == 'top') { echo "selected";}?>>Top</option>
			<option value="bottom" <? if($fetchrow->ad_postion == 'bottom') { echo "selected";}?>>Bottom</option>
			<option value="brands" <? if($fetchrow->ad_postion == 'brands') { echo "selected";}?>>Brands</option>
			</select>
			</td>
			</tr>
			<tr>
			...
			<option value="">Select</option>
			<option value="home" <? if($fetchrow->ad_page == 'home') { echo "selected";}?>>Home</option>
			<option value="listing" <? if($fetchrow->ad_page == 'listing') { echo "selected";}?>>Listing</option>
			<option value="details" <? if($fetchrow->ad_page == 'details') { echo "selected";}?>>Details</option>
			</select>
			</td>
			</tr>
			<tr>
			<td>Ads Order</td><td><input type="text" name="ad_place" value="<?=$fetchrow->ad_place?>" /></td>
			</tr>
			<tr>
