...
<div class="hidden iswf" >
	<table style="width:100%; margin:0;">		
		...
		$select_attribute = mysql_query("SELECT * FROM attribute WHERE attribute_required=1 and attribute_id='$aat' ORDER BY attribute_name*1 ASC");
		$i =1;
		$select_attri=mysql_fetch_array($select_attribute);
...
var addDiv = $('#addinputstocka');
        ...
        $('#addNewstocka').live('click', function() {
			   ...
			   var cnt = $("#att_number").val();
                ...
                $('<p style="width:100%; margin:0; padding:0;"><table style="width:100%; margin:0;"><tr><td align="center" width="25%"><? $select_attribute = mysql_query("SELECT * FROM attribute WHERE attribute_required=1 and attribute_id='$aat' ORDER BY attribute_name*1 ASC");$i =1;$select_attri=mysql_fetch_array($select_attribute);$id = $select_attri['attribute_id'];?><input type="hidden" value="<?=$select_attri['attribute_id']?>" name="stock_id[]" /><select required name="stock_option_id[]"><option value="">Select <?=$select_attri['attribute_name']?></option><? $select_sub_attribute = mysql_query("SELECT * FROM attribute_option WHERE attribute_id='{$select_attri['attribute_id']}' ORDER BY attribute_option_name ASC");  while($row=mysql_fetch_array($select_sub_attribute)):?><option value="<?=$row['id']?>"><?=$row['attribute_option_name']?></option><? endwhile; ?></select><? $i++;?>	<input type="hidden" name="attribute_size" value="<?=$i-1?>"></td><td  width="22%"><input required type="text" name="stock_price[]" id="stock_price" /></td><td align="center" width="22%"><input type="text" name="stock_dis_price[]" id="stock_dis_price" /></td><td width="15%"><input required type="text" name="stock_stock[]" id="stock_stock" /></td><td width="35%"><a href="#" id="remNewstock1" class="green_button">Delete Active</a></td></tr></table></p>').appendTo(addDiv);
                i++;
                return false;
        });
