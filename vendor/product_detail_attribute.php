...
$user = new Admin();
$currentpage = 'product_detail_attribute.php';
$currentpagetitle = 'product detail attribute';
$getdata = $get->get_active_data($_REQUEST['id'],'product_detail_attribute');
$fetchrow = mysql_fetch_object($getdata);
$getrole = $get->get_page_role($_REQUEST['role']);
$page = $_REQUEST['page'];

if(isset($_POST['add'])){
$add = $set->add_product_detail_attribute();
}

if(isset($_POST['edit'])){
$edit = $set->update_product_detail_attribute($_REQUEST['id']);
}

if(isset($_POST['doaction']))
{
	extract($_POST);
	$add = $set->do_action($action,$ids,'product_detail_attribute');
}
...
<div class="dashbox-main-div">
	...
	<?php if($msg != "") { echo '<p class="success">'.$msg.'</p>'; } ?>
		...
		<tr>
		<td width="45%;">Product Group Name <span class="color_red">*</span></td>
...
<?
$selectcat = mysql_query("SELECT * FROM product_detail_attribute ORDER BY id ASC LIMIT $start, $limit");
?>

<table width="100%" style="padding:0; margin:0">
<tr>
	<td align="center"><input type="checkbox"   id="selecctall"  /></td>
    <td><strong>Product Attribute Name</strong></td>	
