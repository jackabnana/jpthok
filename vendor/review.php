...
$user = new Admin();
$currentpage = 'review.php';
$currentpagetitle = 'Review';
$getdata = $get->get_active_data($_REQUEST['id'],'review');
...
if(isset($_POST['doaction'])){
	extract($_POST);
$add = $set->do_action($action,$ids,'review');
}
...
if($_REQUEST['status'])
{
	$selectreview .= " WHERE status='{$_REQUEST['status']}' ";	
}
...
<tr>
	...
	<td><a href="<?=$site_url?>/detail/<?=$product_id?>/" target="_blank"><?=substr($get->get_product_name($product_id),0,40)?></td>
	<td><?=$review_msg?></td>
	<td><?=$review_name?></td>	
	<td><?=$review_email?></td>	
    <td>
...
<? endif; ?>
	...
	<?=$get->get_pagination('review',$currentpage,$page,$start,$limit)?>
<div  class="cboth"></div>
</form>
	</div>
