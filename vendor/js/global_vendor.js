...
$("#register_username").on('keyup keydown mouseUp mousedown onBlur',(function(e) {
	
username = $('#register_username').val();
$("#useruniqueval").val(1);

if(username.length > 1 && ($('#register_username').val().match("^[a-z0-9'.\s]{1,50}$"))){
	$('#msg_show').show();
	$('#username_error_msg').focus().html('');
	
	$.ajax(
	{
		url : global_url+"action.php?username="+username,
		type: "POST",
		data : 'HTML',
		beforeSend: function() {		
		$('#userloading').show();
...
$("#step_one").click(function(e)
{
		...
		var useruniqueval = $('#useruniqueval').val();
	...
	if(register_username == ''){
		$("#username_error_msg").html('<span class="mgserror"><i class="fa-exclamation-triangle fa"></i>Please enter username.</span>');
		$('#login-response').focus().html('');
		$('#register_username').focus();		
		return false;
	}else if(useruniqueval=='' || useruniqueval > 0){
		$("#username_error_msg").html('<span class="mgserror"><i class="fa-exclamation-triangle fa"></i>Only enter alphnumeric without space and special characters.</span>');
		$('#login-response,#msg_show').focus().html('');
		$('#register_username').focus();		
		return false;
	}else if(full_name=='')
		{
...
$("#step_two").click(function(e)
{
		...
		check_login_terms = $('input[name=check_login_terms]:checked').length;
		...
		else if(user_password == '') 
		{
			...
			$('#user_password').focus();
			return false;
		}else if(check_login_terms <=0){
			$("#login_response").html('<div class="mgs error col-md-40"> Please accept our terms</div>');
			$('#check_login_terms').focus();
			return false;			
		}
		else 
		{ 	
