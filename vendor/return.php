...
<?
//$sql = "SELECT OD.id as oid,OD.*,OCD.* FROM order_details as OD JOIN order_customer_details as OCD ON OD.orderid = OCD.orderid JOIN return_order AS RO ON OD.orderid = RO.order_id";

$sql = "SELECT OPD.id as oid,OPD.*,OCD.* FROM order_prodcut_details AS OPD JOIN order_customer_details as OCD ON OPD.orderid = OCD.orderid JOIN return_order AS CO ON (OPD.orderid = CO.order_id AND OPD.id = CO.detail_id) WHERE OPD.vendor_id = '".$_SESSION['vendor_id']."'  GROUP BY OPD.orderid";
...
<tr>
	...
	<td><strong>Return Comment</strong></td>
...
if($count < 1)
{
	?><tr><td align="center" colspan="9">No result(s) found.</td></tr><?
}
$x=1;
...
<tr>
	...
	?>
	<td><?=$get->return_date($orderid)?></td>
	<td><?=$get->return_reason($orderid)?></td>
	<td><?=$get->return_comment($orderid)?></td>
	<td><?=$payment_method?></td>
	<td>Return</td>
	<td>
	<?
	if($_REQUEST['action']=='track'){ ?>
	<a href="<?=ADMIN_PATH?>/track-order.php?role=view-invoice&page=<?=$page?>&id=<?=$orderid?>">View Order</a>
	<? } else { ?>
	<a href="<?=ADMIN_PATH?>/<?=$currentpage?>?role=view-invoice&page=<?=$page?>&id=<?=$orderid?>">View Order</a>
	<? } ?>
	</td>	
</tr>
<? 
