...
<tr>	
	...
	Pin: <?=$billzip?><br/>
	MOBILE: <?=$billcontact?><br />
	Barcode: <?$data = $_REQUEST['id']?>
...
<span style="float:left;">Invioce No: <br/>
<?$invoice_no = $get->generate_invoice_no($_REQUEST['did'])?>
<img style="-webkit-user-select: none" src="http://generator.barcodetools.com/barcode.png?gen=0&data=<?=$invoice_no?>&bcolor=FFFFFF&fcolor=000000&tcolor=000000&fh=10&bred=0&w2n=2.5&xdim=2&w=240&h=50&debug=1&btype=7&angle=0&quiet=1&balign=2&talign=0&guarg=1&text=1&tdown=1&stst=1&schk=0&cchk=1&ntxt=1&c128=0">
</span>
...
<?
	$selectcart = mysql_query("SELECT * FROM order_prodcut_details WHERE orderid='{$_REQUEST['id']}' AND id = '{$_REQUEST['did']}'  AND vendor_id = '".$_SESSION['vendor_id']."'");
	$qtysum = 0;
	...
	?>
<tr>
<td><?=$get->get_product_name($prod_id)?>(<?=$get->get_product_code($prod_id)?>)<br/>
<? 
$aid = $get->get_order_attribute_id($_REQUEST['id'],$_SESSION['vendor_id'],$prod_id,'');
if($aid > 0){
echo $get->get_attribute_name($aid)?>: <?=$get->get_attribute_option_name($attribute_option_id);
}
?>
</td>
...
$deliveryamount = $get->get_total_delivery_charges($orderid); 
...
<?
   ...
   {
	   ?>
		<td colspan="4"><b><?=$get->get_store_name($_SESSION['vendor_id']);?></b><br />
		<?=$get->get_store_address($_SESSION['vendor_id']);?>
		</td>
	    <?
   }
   else
   {   
		?>
		<td colspan="4"><b>FKLICK FAB PVT LTD</b><br />
		A-41, Vishwas Park,<br />
		Uttam Nagar, New Delhi - 110059<br />
		<b>CITY</b>: Delhi<br />
		<b>STATE</b>: Delhi<br />
		<b>PIN</b>: 110059<br />
		<b>PHONE</b>: 09212808000<br />
		<b>COMPANY'S VAT / TIN NO.</b>: 07127108852 <br />w.e.f. 10/02/2016<br />
		<b>COMPANY'S CST NO.</b>: 07127108852<br />
		<b>COMPANY'S PAN NO.</b>: AACCF7378K<br />
		</td>
	   <?
   }
   ?>
...
<?
   ...
   {
	   ?>
		<p><b><?=$get->get_store_name($_SESSION['vendor_id']);?></b></p><br />
		<p><?=$get->get_store_address($_SESSION['vendor_id']);?></p>
	    <?
   }
   else
   {
...
<tr class="heading">
      ...
      <td style="width:5%" align="right"><b>Quantity</b></td>
      <td style="width:12%" align="right"><b>Unit Price</b></td>
	  <td style="width:12%" align="right"><b>CST / VAT</b></td>
	  <td style="width:5%" align="right"><b>S.Bharat</b></td>
	  <td style="width:5%" align="right"><b>Discount</b></td>
      <td style="width:10%" align="right"><b>Total</b></td>
</tr>
<?
	$selectcart_new = mysql_query("SELECT * FROM order_prodcut_details WHERE orderid='{$_REQUEST['id']}' AND id = '{$_REQUEST['did']}'  AND vendor_id = '".$_SESSION['vendor_id']."'");
	while($raw_new=mysql_fetch_array($selectcart_new)):
	extract($raw_new);
	if($combo == 0){
	$total_new = $subtotal*$qty;
	...
	$service_tax = $get->get_service_tax();	
	$sTax = (($subtotal*$service_tax)/100);
	
	$swachh_bharat = $get->get_swachh_bharat();
	$sBtax = (($subtotal*$swachh_bharat)/100);
?> 
<tr>
<td><?=$get->get_product_name($prod_id)?>(<?=$get->get_product_code($prod_id)?>)<br/>
<? 
...
<td align="right" width="70">Rs. <?=$subtotal-$cstORvat?></td>
<td align="right" width="70">Rs. <?=$sTax?></td>
<td align="right" width="20">Rs. <?=$sBtax?></td>
<td align="right" width="20">---</td>
<td align="right" width="40">Rs. <?=$total_new?></td>
</tr>
<? endwhile; ?>
<tr>
...
{
	?>
	<td align="right">Rs. <?=$total_am_new+$delivery_charges?></td>
	<?
}
else
{
