...
//////// Forget Password /////////////
	...
	if($count > 0){
		...
		www.ezdayshop.com
		";
		if(mail($to,'ezdayshop Password Recover',$msg,$from)){
	...
	public function change_password(){
		...
		if($count > 0)
		{
			if($new_pass != $re_pass)
			{
				$url = SITE_URL.'/change-password.'.PAGE_EXT.'?ownername=f9e54aa256b98af7abe9d1dd35e545cc&msg=error';
			    header('location: '.$url.'');
			}
			else
			{
				mysql_query(" UPDATE user SET password='$new_pass' WHERE user_id='$user_id' ");
				$url = SITE_URL.'/change-password.'.PAGE_EXT.'?ownername=f9e54aa256b98af7abe9d1dd35e545cc&msg=success';
				header('location: '.$url.'');
			}
		}
		else
		...
		else if($payment_method == 'COD')
		{
			...
			if(mysql_query($sql_detail))
			{
				...
				for($i=0;$i<$this->get_cart_qty();$i++)
				{
					...
					<tbody>
					<tr>
					<td valign='middle' align='left' height='auto'  style='background-color:#efefef;padding:10px;margin:0'><a style='text-decoration:none;outline:none;font-size:13px' href='http://www.ezdayshop.com/' target='_blank'> <img border='0' src='http://www.ezdayshop.com/upload/comman/logo_1632837691.png' alt='ezdayshop' style='border:none; padding-top: 5px;' class='CToWUd'> </a></td>
					...
					<tr>
					<td style='width:30px; margin-left:10px;'><a href='https://www.facebook.com/ezdayshop' target='_blank'><img src='http://www.ezdayshop.com/images/facebook.png' width='30' height='30' alt='facebook'></a></td>
					<td style='width:30px; margin-left:10px;'><a href='https://twitter.com/ezdayshop' target='_blank'><img src='http://www.ezdayshop.com/images/twiiter.png' width='30' height='30' alt='twiiter'></a></td>
					<td style='width:30px; margin-left:10px;'><a href='https://plus.google.com/+ezdayshop/posts' target='_blank'><img src='http://www.ezdayshop.com/images/Google-plus-icon.png' width='30' height='30' alt='linkedin+'></a></td>
					</tr>
					</table></td>
					</tr>
					</tbody>
					...
					<br>
					<p style='padding:0;margin:0;color:#565656;font-size:13px'>Greeting from ezdayshop!</p>
					<br>
					<p style='padding:0;margin:0;color:#565656;font-size:13px'> We have received your order <a style='text-decoration:none' alt='ezdayshop.com' href='#' target='_blank'><span style='color:#666;font-size:13px'>$orderid</span></a> </p></td>
					</tr>
					<tr>
					...
					<tbody>
					<tr>
					<td valign='middle' align='left' rowspan='2' style='white-space:nowrap;padding-right:5px;font-size:13px; margin-bottom:10px;'>Seller: ezdayshop.com</td>
					...
					<tbody>
					<tr>
					<td valign='top' align='center' style='text-align:center;background-color:#f9f9f9;display:block;margin:0 auto;clear:both;padding:15px 40px' bgcolor=''><p style='padding:0;margin:0 0 7px 0'> <a title='ezdayshop.com' style='text-decoration:none;color:#565656' href='http://www.ezdayshop.com/' target='_blank'><span style='color:#565656;font-size:13px'>ezdayshop.com</span></a> </p>
					...
					$subject = 'Order at ezdayshop.com';
					$headers = "MIME-Version: 1.0" . "\r\n";
					$headers .= "Content-type:text/html;charset=UTF-8" . "\r\n";
					$headers .= 'From: <info@ezdayshop.com>' . "\r\n";
					...
					$subject1 = 'Order received at ezdayshop.com.';
...
/******************Search Functions*********************/
        ...
        public function change_order_status()
	{
		...
		extract($data);
		$select_noti = mysql_query(" SELECT * FROM product_noti WHERE email='$notify_me' and prod_id='$notify_prod_id' ");
		$count = mysql_num_rows($select_noti);
		if($count == 0){
		$sql = "INSERT INTO product_noti SET 
			prod_id='$notify_prod_id',
			email='$notify_me',
			status='active'";
			mysql_query($sql);
		}
