...
function get_combo_amount($cid)
{
   ...
   public function product_delivery_for_order($oid,$id)
    {
        $select = mysql_query("SELECT * FROM product WHERE prod_id = '$id'");
        $row=mysql_fetch_array($select);
        $day = $row['product_delivery_day']+1;
  ...
  public function get_recent_product($limit)
  {
       if($limit !='') { $limitadd = " LIMIT $limit"; } else { $limitadd = ""; }
       $ip = $this->get_user_ip();
       $sql = "select RVP.*,P.* from recent_viewproduct as RVP JOIN product as P ON RVP.prod_id=P.prod_id where RVP.status='active' and RVP.ip_address='$ip' ORDER BY rand() $limitadd ";
       $query = mysql_query($sql);
       return $query;
   }
...
/******************Ads Get Functions*********************/	
	...
	// Get Add
	function get_ads($ad_postion,$ad_page){
		$selectads = " SELECT * FROM ads WHERE ad_postion='$ad_postion' and ad_page='$ad_page' and status='active' ORDER BY RAND() LIMIT 1 ";
...
/******************Wishlist Get Functions*********************/	
	...
	function get_wishlist($pid,$uid){
		...
		if($count == 1){ $active = 'pink-button-active'; }
		return $active;
	}
