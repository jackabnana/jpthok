...
$getdata = mysql_query("SELECT * FROM product WHERE id='{$_REQUEST['id']}' and prod_id='{$_REQUEST['request_prod_id']}' AND vendor_id = '{$_SESSION['vendor_id']}' ");
...
<link href="<?=VENDOR_PATH?>/css/select2.css" rel="stylesheet"/>
<link href='http://fonts.googleapis.com/css?family=Ubuntu:300,400' rel='stylesheet' type='text/css'>
<script  src="<?=VENDOR_PATH?>/js/jquery.min.js"></script>
...
function checkBtn()
{
	...
	var checked=false;
	var elements = document.getElementsByName("cat_id[]");
...
<div id="calculatorPOSE86J3PUYNXD7C" class="dropdown-relative calculator_dropdown jq-dropdown dropdown-tip dropdown-anchor-right">
</div>	
...
<h2><span>Manage <?=$currentpagetitle?></span>
	<p> 
	<a href="<?=VENDOR_PATH?>/<?=$currentpage?>?role=view&page=<?=$_REQUEST['page']?>" class="red_button fright">Back</a>
	</p>
    <div class="cboth"></div>
</h2>
</div>
<div class="dashbox-main-div">
	<?php if($msg != "") { echo '<p class="success">'.$msg.'</p>'; } ?>
		...
		<!--Add product-->
		<form action="" method="post" enctype="multipart/form-data" onsubmit="return checkBtn()">
		<h2><?=$getrole?> Product
		<input type="submit" name="<?=$getrole?>" class="green_button fright" value="<? if($getrole == 'edit') { $getrole = 'Update'; }?> <?=$getrole?> Product " />
		<!--<input type="submit" name="process_done" value="Done">-->
		</h2>
		...
		<td width="50%">
		<input type="checkbox" name="best_sales[]" value="BS" <? if(in_array('BS',$best_sales)){ echo 'checked'; } ?> /> Best Sales
		<input type="checkbox" name="best_sales[]" value="PP" <? if(in_array('PP',$best_sales)){ echo 'checked'; } ?> /> Popular Products
		<input type="checkbox" name="best_sales[]" value="VPC" <? if(in_array('VPC',$best_sales)){ echo 'checked'; } ?> /> Value Packs & Combos
		...
		<td>Is Active <span class="color_red">*</span></td><td>
			...
			<option <?if($fetchrow->status == 'active'){ ?>selected<? }?> value="active" <?=$selectactive?>>Active</option>
			<option <?if($fetchrow->status == 'deactive'){ ?>selected<? }?> value="deactive" <?=$selectdeactive?>>Deactive</option>
			</select>
		</td>
		</tr>
		...
		<?php 
			$ship_query = mysql_query("SELECT * FROM shipping_charges ORDER BY weight ASC")or die(mysql_error());
			while($ship_result =mysql_fetch_array($ship_query)){			
			$array[] = $ship_result['weight'];
			}
			?>	
		
		<tr>
		<td>Weight (In Gram) <span class="color_red">*</span></td><td>			
			...
			for($i=0; $i<$count; $i++){				
			?>			
			<option value="<?=$array[$i]?>" <?php if($fetchrow->product_weight == $array[$i]){echo "selected";}?>>			
			<?php if($i==0){echo "0";}else{echo ($array[$i-1]+1);} echo "-".($array[$i]);?> gm			
			</option>
			<?php } ?>			
			</select>
		</td>
		...
		<?php /* ?>
		<div class="tabContent" id="stock" style="display:none;">
		<div>
		...
		<!-- Stock group -->
		 ...
		 <div class="selectContainer fleft" style="width:20%;">
		        <select id="theSelect" style="width:200px;" onChange="showstock(this.value);">
		            <option value="">- Select -</option>
					<? 
					$select_group = mysql_query("SELECT * FROM attribute WHERE status='active' AND attribute_id='987889555' ");
					while($row_group=mysql_fetch_array($select_group))
					{
					
						$select_product_atrr = mysql_query("SELECT * FROM product_attribute WHERE prod_id='{$_REQUEST['request_prod_id']}' and attribute_id!=0 and status='active'");
						$select_product_attr_row=mysql_fetch_array($select_product_atrr);
						echo $select_product_attr_row['attribute_id'].'='.$row_group['attribute_id'];
						if($select_product_attr_row['attribute_id'] == $row_group['attribute_id']){
							$selecteda = "selected";
						} else {
							$selecteda = "";
						}
						?>
			<!-- Only size is taken as attribute-->
		
			   <option value="<?=$row_group['attribute_id']?>" <?=$selecteda?>><?=$row_group['attribute_name']?></option>
			<?	 } ?>
		        </select>
		 </div>
	...
	<?
	$x=0;
echo "SELECT * FROM product_attribute WHERE prod_id='{$_REQUEST['request_prod_id']}' and attribute_id!=0 and status='active'";
	$select_product_groupa = mysql_query("SELECT * FROM product_attribute WHERE prod_id='{$_REQUEST['request_prod_id']}' and attribute_id!=0 and status='active'");
	while($select_product_group_row=mysql_fetch_array($select_product_groupa))
	{	

			$att = mysql_query("SELECT * FROM attribute WHERE attribute_id = '".$select_product_group_row['attribute_id']."' ");
		...
		<input type="hidden" value="<?=$select_product_group_row['id']?>" name="product_attribute_id[]" id="product_attribute_id">
		
		<input type="hidden" name="stock_id[]" value="<?=$select_product_group_row['attribute_id']?>">
		<input type="hidden" name="stock_group_id" id="stock_group_id" value="<?=$select_product_group_row['attribute_id']?>">
		<td  width="22%">
	...
	?>
		...
		<div class="tabContent" id="attribute" style="display:none;">
		<div>
		<table style="width:100%; margin:0;">
			<?
			$a=1;			
			$select_attribute = mysql_query("SELECT * FROM attribute ");
			...
			<b><?=$select_attri['attribute_name']?></b>
				...
				?>
				<option value="<?=$row['id']?>">
				<?=$row['attribute_option_name']?></option>
				<? endwhile; ?>
				</select>
			</td>
...
<h2><span>Manage <?=$currentpagetitle?></span>
	<p> 
	<a href="<?=VENDOR_PATH?>/<?=$currentpage?>?role=add" class="green_button fright">Add <?=$currentpagetitle?></a>
	</p>
    <div class="cboth"></div>
</h2>
</div>
...
<?
 ...
 $sql = " SELECT P.id as pid,P.status as p_status,P.*,C.* From product as P, 
	category as C  WHERE P.vendor_id = '".$_SESSION['vendor_id']."' ";
...
<tr>
	...
	<td><strong>Commission Chart</strong></td>
...
<tr>
	...
	<a href="#0" class="edit">
	<img src="images/comm.jpeg" width="35" onclick="show_filter(<?=$prod_id?>)" /> 
	</a>
	</td>

	<td width="50" align="center">
	<a href="<?=VENDOR_PATH?>/<?=$currentpage?>?role=edit&page=<?=$page?>&id=<?=$pid?>&request_prod_id=<?=$prod_id?>" class="edit">
...
?>
<tr>
	<td align="center"  colspan="9">No Recode Found!!</td>

</tr>
<? endif; ?>
</table>
	<?=$get->get_pagination_vendor('product',$currentpage,$page,$start,$limit,$_SESSION['vendor_id'])?>
<div  class="cboth"></div>
</form>
	</div>
...
function showstock_nagendra(str) 
{
	//alert(str);
...
<script src="<?=VENDOR_PATH?>/js/jquery-1.8.0.min.js"></script>
<script src="<?=VENDOR_PATH?>/js/select2.js"></script>
<script>
function show(id){
	//alert(id);
	$('#'+id).toggle();
