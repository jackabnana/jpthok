	 			...
	 			<div class="row bg-white shadow-1 margin-top-20px margin-bottom-20px" style=" border-bottom: 1px solid #CCC;">
					...
					?>
	 					<li class="product">
						  <? if($url !=''){ ?>
	 						<div class="thumb">
							<a href="<?=$site_url?>/detail/pid-<?=$prod_id?>/<?=$url?>"><img src="<?=$site_url?>/upload/product/thumb/th_<?=$get->get_single_product_img($prod_id)?>"></a>
							</div>
	 						<div class="product-disc">
	 							<h4 class="order-name"><a href="<?=$site_url?>/detail/pid-<?=$prod_id?>/<?=$url?>"><?=$get->get_product_name($prod_id)?></a></h4>
